...
You will also need a Sql Server, run it with docker:
    ...
    docker run --platform linux/amd64 --cap-add SYS_PTRACE  -e "ACCEPT_EULA=Y" -e "MSSQL_SA_PASSWORD=p4ssw0rD" -p 1433:1433  mcr.microsoft.com/mssql/server -d
    sqlcmd -S localhost -d master -C -P p4ssw0rD -U sa
